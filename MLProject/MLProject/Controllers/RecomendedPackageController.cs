...
namespace MLProject.Controllers
{
    ...
    public class RecomendedPackageController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<KeyValuePair<int,PackageScoreAndPrice>>>> GetRecommendedPackages([FromQuery]  List<int>AnalysisList, int TakeOnlyNums=5,   bool LowestCost =true)
        {
            if (AnalysisList == null)
                return BadRequest("Analysis List Must Be Assigned!");
            if (AnalysisList.Count == 0) return BadRequest("Analysis List Must Be Full");
            ...
            return RecommendedPackages.Count != 0 ? Ok(RecommendedPackages.ToList()) : NotFound("No Package Contains Any Of This Analysis In The List!");
